...
<script type="text/javascript">
	function toSub(){
		var outboundTourism_desc=$("#outboundTourism_desc").val();
		if(outboundTourism_desc.trim().length==0){
			alert("旅游详情不能为空，请重新输入");
			return false;
		}
		$("#myForm").submit();
	}
				...
				<div class="caption">
					<i class="fa fa-gift"></i>热门景点编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="OutboundTourismUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="景点描述" name="outboundTourism_desc" id="outboundTourism_desc" value="${outboundTourism.outboundTourism_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="outboundTourism_id" name="outboundTourism_id" type="hidden" value="${outboundTourism.outboundTourism_id }"/>
								</div>
							</form>
						</div>
					</div>
