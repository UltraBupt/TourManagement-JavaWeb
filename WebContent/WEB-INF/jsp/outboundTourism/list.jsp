...
<script type="text/javascript">
	//上传头像
	function upload(outboundTourism_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'OutboundTourismUploadServlet.shxt?outboundTourism_id='+outboundTourism_id,
			   width:600,
			   height:170,
			   onclose:function(){
	...
	//编辑信息
	function updateOutboundTourism(outboundTourism_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'OutboundTourismUpdateServlet.shxt?outboundTourism_id='+outboundTourism_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								<form action="OutboundTourismServlet.shxt" method="post" id="myForm">
								...
								<c:forEach items="${pageBean.data }" var="outboundTourism" varStatus="vs">
									<tr class="active">
									<td style="width: 91px; height: 91px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${outboundTourism.outboundTourism_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${outboundTourism.outboundTourism_desc }
									</td>
									<td>
										 ${outboundTourism.outboundTourism_name }
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${outboundTourism.outboundTourism_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateOutboundTourism(${outboundTourism.outboundTourism_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
