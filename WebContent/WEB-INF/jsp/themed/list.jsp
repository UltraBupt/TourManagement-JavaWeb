...
<script type="text/javascript">
	...
	//编辑信息
	function updateThemed(themed_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'ThemedUpdateServlet.shxt?themed_id='+themed_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(themed_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'ThemedUploadServlet.shxt?themed_id='+themed_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(themed_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeThemedStatusServlet.shxt",{themed_id:themed_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.themed_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="ThemedServlet.shxt" method="post" id="myForm">
									<input type="text" name="themed_name" id="themed_name" value="${queryThemed.themed_name }" placeholder="请输入主题名称" maxlength="15">
									<select name="themed_status" id="themed_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryThemed.themed_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryThemed.themed_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addThemed()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="themed" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${themed.themed_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${themed.themed_name }
									</td>
									...
									<td>
										 ${themed.themed_address }
									</td>
									<td>
										 ${themed.themed_desc }
									</td>
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${themed.themed_id}',this)">
										 <c:if test="${themed.themed_status eq '1' }">存在</c:if>
										 <c:if test="${themed.themed_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${themed.themed_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateThemed(${themed.themed_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
