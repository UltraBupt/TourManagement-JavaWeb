...
<script type="text/javascript">
	//上传头像
	function upload(aroundTicket_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'AroundTicketUploadServlet.shxt?aroundTicket_id='+aroundTicket_id,
			   width:600,
			   height:170,
			   onclose:function(){
	...
	//编辑信息
	function updateAroundTicket(aroundTicket_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'AroundTicketUpdateServlet.shxt?aroundTicket_id='+aroundTicket_id,
			   width:600,
			   height:240,
			   onclose:function(){
				  if(this.returnValue=="success"){
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								<form action="AroundTicketServlet.shxt" method="post" id="myForm">
								...
								<c:forEach items="${pageBean.data }" var="aroundTicket" varStatus="vs">
									<tr class="active">
									<td style="width: 91px; height: 91px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${aroundTicket.aroundTicket_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${aroundTicket.aroundTicket_desc }
									</td>
									<td>
										 ${aroundTicket.aroundTicket_price }
									</td>
									<td>
										 ${aroundTicket.aroundTicket_name }
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${aroundTicket.aroundTicket_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateAroundTicket(${aroundTicket.aroundTicket_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
