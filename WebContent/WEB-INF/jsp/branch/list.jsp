...
<script type="text/javascript">
	//添加部门
		...
		//编辑信息
		function upload(branch_id){
		   var updateDialog = top.dialog({
			   title:'部门编辑',
			   url:'BranchUpdateServlet.shxt?branch_id='+branch_id,
			   width:600,
			   height:265,
			   onclose:function(){
				  if(this.returnValue=="success"){
		...
		//变更状态
        function toChangeStatus(branch_id,obj){
        	var txt = $.trim($(obj).html());
        	if(window.confirm("您确定要改变状态么")){
        		 $.post("ChangeBranchStatusServlet.shxt",{branch_id:branch_id},function(data){
			...
			<div class="portlet box green">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								<form action="SectionListServlet.shxt" method="post" id="myForm">
									<input type="text" name="branch_name" id="branch_name" value="${queryBranch.branch_name }" placeholder="请输入名称" maxlength="15">
									<select name="branch_status" id="branch_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryBranch.branch_status eq '1' }">
												selected
											</c:if>
										>可用</option>
										<option value="0"
											<c:if test="${queryBranch.branch_status eq '0' }">
												selected
											</c:if>
										>禁用</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs green" onclick="addBranch()">添加部门</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="branch" varStatus="vs">
									<tr class="success">
									...
									<td>
									<a href="javascript:void(0)" style="color:green" onclick="toChangeStatus('${branch.branch_id}',this)">
										<c:if test="${branch.branch_status eq '1' }">可用</c:if>
										<c:if test="${branch.branch_status eq '0' }">禁用</c:if>
									</a>
									</td>
									<td>
										<shxt class="btn btn-circle btn-xs green" onclick="upload(${branch.branch_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
