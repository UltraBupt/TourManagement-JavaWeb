...
<script type="text/javascript">
	...
	function updateShip(ship_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'ShipUpdateServlet.shxt?ship_id='+ship_id,
			   width:600,
			   height:350,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(ship_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'ShipUploadServlet.shxt?ship_id='+ship_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(ship_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeShipStatusServlet.shxt",{ship_id:ship_id},function(data){
...
<body>
			<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-comments"></i>轮船票管理
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="ShipServlet.shxt" method="post" id="myForm">
									<input type="text" name="ship_point" id="ship_point" value="${queryShip.ship_point }" placeholder="请输入机场" maxlength="15">
									<select name="ship_status" id="ship_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryShip.ship_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryShip.ship_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addShip()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="ship" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${ship.ship_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${ship.ship_from }
									</td>
									...
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${ship.ship_id}',this)">
										 <c:if test="${ship.ship_status eq '1' }">存在</c:if>
										 <c:if test="${ship.ship_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${ship.ship_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateShip(${ship.ship_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
