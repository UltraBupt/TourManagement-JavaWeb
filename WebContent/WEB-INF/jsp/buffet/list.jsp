...
<script type="text/javascript">
	...
	//编辑信息
	function updateBuffet(buffet_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'BuffetUpdateServlet.shxt?buffet_id='+buffet_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(buffet_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'BuffetUploadServlet.shxt?buffet_id='+buffet_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(buffet_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeBuffetStatusServlet.shxt",{buffet_id:buffet_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.buffet_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="BuffetServlet.shxt" method="post" id="myForm">
									<input type="text" name="buffet_name" id="buffet_name" value="${queryBuffet.buffet_name }" placeholder="请输入自助名称" maxlength="15">
									<select name="buffet_status" id="buffet_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryBuffet.buffet_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryBuffet.buffet_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addBuffet()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="buffet" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${buffet.buffet_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${buffet.buffet_name }
									</td>
									...
									<td>
										 ${buffet.buffet_address }
									</td>
									<td>
										 ${buffet.buffet_desc }
									</td>
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${buffet.buffet_id}',this)">
										 <c:if test="${buffet.buffet_status eq '1' }">存在</c:if>
										 <c:if test="${buffet.buffet_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${buffet.buffet_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateBuffet(${buffet.buffet_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
