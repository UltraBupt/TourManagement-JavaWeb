...
<script type="text/javascript">
	function toSub(){
		var fk_partnerType_id=$("#fk_partnerType_id").val();
		var partner_name=$("#partner_name").val();
		var partner_position=$("#partner_position").val();
		var partner_hobby=$("#partner_hobby").val();
		var partner_desc=$("#partner_desc").val();
		if(fk_partnerType_id.trim().length==0){
			alert("供应商类型不能为空，请选择");
			return false;
		}
		if(partner_name.trim().length==0){
			alert("供应商名称不能为空，请重新输入");
			return false;
		}
		if(partner_position.trim().length==0){
			alert("供应商职位不能为空，请重新输入");
			return false;
		}
		if(partner_hobby.trim().length==0){
			alert("供应商爱好不能为空，请重新输入");
			return false;
		}
		if(partner_desc.trim().length==0){
			alert("供应商描述不能为空，请重新输入");
			return false;
		}
		$("#myForm").submit();
	}
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 供应商信息
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="PartnerUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="col-md-4" style="height:150px">
										<select name="fk_partnerType_id" id="fk_partnerType_id" class="form-control input-xlarge select2me" data-placeholder="请选择要分配的类型">
											<option value="">全部</option>
												<c:forEach items="${partnerType_list }" var="partnerType">
												<option value="${partnerType.partnerType_id }"
												<c:if test="${partner.fk_partnerType_id==partnerType.partnerType_id }">selected</c:if>
									...
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="供应商名称" name="partner_name" id="partner_name" value="${partner.partner_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="供应商职位" name="partner_position" id="partner_position" value="${partner.partner_position }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="供应商爱好" name="partner_hobby" id="partner_hobby" value="${partner.partner_hobby }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="供应商描述" name="partner_desc" id="partner_desc" value="${partner.partner_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn yellow" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="partner_id" name="partner_id" type="hidden" value="${partner.partner_id }"/>
								</div>
							</form>
