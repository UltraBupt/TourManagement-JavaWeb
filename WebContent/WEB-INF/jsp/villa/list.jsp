...
<script type="text/javascript">
	...
	//编辑信息
	function updateVilla(villa_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'VillaUpdateServlet.shxt?villa_id='+villa_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(villa_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'VillaUploadServlet.shxt?villa_id='+villa_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(villa_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeVillaStatusServlet.shxt",{villa_id:villa_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.villa_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="VillaServlet.shxt" method="post" id="myForm">
									<input type="text" name="villa_name" id="villa_name" value="${queryVilla.villa_name }" placeholder="请输入别墅名称" maxlength="15">
									<select name="villa_status" id="villa_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryVilla.villa_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryVilla.villa_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addVilla()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="villa" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${villa.villa_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${villa.villa_name }
									</td>
									...
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${villa.villa_id}',this)">
										 <c:if test="${villa.villa_status eq '1' }">存在</c:if>
										 <c:if test="${villa.villa_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${villa.villa_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateVilla(${villa.villa_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
