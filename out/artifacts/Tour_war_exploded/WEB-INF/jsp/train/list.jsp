...
<script type="text/javascript">
	...
	function updateTrain(train_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'TrainUpdateServlet.shxt?train_id='+train_id,
			   width:600,
			   height:350,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(train_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'TrainUploadServlet.shxt?train_id='+train_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(train_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeTrainStatusServlet.shxt",{train_id:train_id},function(data){
...
<body>
			<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-comments"></i>火车票管理
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="TrainServlet.shxt" method="post" id="myForm">
									<input type="text" name="train_point" id="train_point" value="${queryTrain.train_point }" placeholder="请输入机场" maxlength="15">
									<select name="train_status" id="train_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryTrain.train_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryTrain.train_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addTrain()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="train" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${train.train_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${train.train_from }
									</td>
									...
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${train.train_id}',this)">
										 <c:if test="${train.train_status eq '1' }">存在</c:if>
										 <c:if test="${train.train_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${train.train_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateTrain(${train.train_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
