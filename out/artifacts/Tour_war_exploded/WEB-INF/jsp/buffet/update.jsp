...
<script type="text/javascript">
	function toSub(){
		var buffet_name=$("#buffet_name").val();
		var buffet_price=$("#buffet_price").val();
		var buffet_address=$("#buffet_address").val();
		var buffet_desc=$("#buffet_desc").val();
		if(buffet_name.trim().length==0){
			alert("请输入名称再进行提交");
			return false;
		}
		if(buffet_price.trim().length==0){
			alert("请输入金额再进行提交");
			return false;
		}
		if(buffet_address.trim().length==0){
			alert("请输入地址再进行提交");
			return false;
		}
		if(buffet_desc.trim().length==0){
			alert("请输入描述再进行提交");
			return false;
		}
		$("#myForm").submit();
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="BuffetUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="名称" name="buffet_name" id="buffet_name" value="${buffet.buffet_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="buffet_price" id="buffet_price" value="${buffet.buffet_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="地址" name="buffet_address" id="buffet_address" value="${buffet.buffet_address }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="详情" name="buffet_desc" id="buffet_desc" value="${buffet.buffet_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="buffet_id" name="buffet_id" type="hidden" value="${buffet.buffet_id }"/>
								</div>
							</form>
						</div>
					</div>
