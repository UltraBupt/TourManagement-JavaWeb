...
<script type="text/javascript">
	//上传头像
	function upload(aroundTourism_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'AroundTourismUploadServlet.shxt?aroundTourism_id='+aroundTourism_id,
			   width:600,
			   height:170,
			   onclose:function(){
	...
	//编辑信息
	function updateAroundTourism(aroundTourism_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'AroundTourismUpdateServlet.shxt?aroundTourism_id='+aroundTourism_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								<form action="AroundTourismServlet.shxt" method="post" id="myForm">
								...
								<c:forEach items="${pageBean.data }" var="aroundTourism" varStatus="vs">
									<tr class="active">
									<td style="width: 91px; height: 91px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${aroundTourism.aroundTourism_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${aroundTourism.aroundTourism_desc }
									</td>
									<td>
										 ${aroundTourism.aroundTourism_name }
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${aroundTourism.aroundTourism_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateAroundTourism(${aroundTourism.aroundTourism_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
