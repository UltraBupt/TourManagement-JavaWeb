...
<script type="text/javascript">
	//上传头像
	function upload(outboundTicket_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'OutboundTicketUploadServlet.shxt?outboundTicket_id='+outboundTicket_id,
			   width:600,
			   height:170,
			   onclose:function(){
	...
	//编辑信息
	function updateOutboundTicket(outboundTicket_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'OutboundTicketUpdateServlet.shxt?outboundTicket_id='+outboundTicket_id,
			   width:600,
			   height:240,
			   onclose:function(){
				  if(this.returnValue=="success"){
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								<form action="OutboundTicketServlet.shxt" method="post" id="myForm">
								...
								<c:forEach items="${pageBean.data }" var="outboundTicket" varStatus="vs">
									<tr class="active">
									<td style="width: 91px; height: 91px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${outboundTicket.outboundTicket_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${outboundTicket.outboundTicket_desc }
									</td>
									<td>
										 ${outboundTicket.outboundTicket_price }
									</td>
									<td>
										 ${outboundTicket.outboundTicket_name }
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${outboundTicket.outboundTicket_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateOutboundTicket(${outboundTicket.outboundTicket_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
