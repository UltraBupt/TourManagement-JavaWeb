...
<script type="text/javascript">
	...
	function updateHotel(hotel_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'HotelUpdateServlet.shxt?hotel_id='+hotel_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(hotel_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'HotelUploadServlet.shxt?hotel_id='+hotel_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(hotel_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeHotelStatusServlet.shxt",{hotel_id:hotel_id},function(data){
...
<body>
			<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-comments"></i>快捷酒店管理
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="HotelServlet.shxt" method="post" id="myForm">
									<input type="text" name="hotel_name" id="hotel_name" value="${queryHotel.hotel_name }" placeholder="请输入快捷酒店名称" maxlength="15">
									<select name="hotel_status" id="hotel_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryHotel.hotel_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryHotel.hotel_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addHotel()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="hotel" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${hotel.hotel_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${hotel.hotel_name }
									</td>
									...
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${hotel.hotel_id}',this)">
										 <c:if test="${hotel.hotel_status eq '1' }">存在</c:if>
										 <c:if test="${hotel.hotel_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${hotel.hotel_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateHotel(${hotel.hotel_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
