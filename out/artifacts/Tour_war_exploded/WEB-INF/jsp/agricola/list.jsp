...
<script type="text/javascript">
	//添加
	function addAgricola(){
		   var addDialog = top.dialog({
			   title:'添加',
			   url:'AgricolaAddServlet.shxt',
			   width:600,
			   height:300,
			   onclose:function(){
	...
	//编辑信息
	function updateAgricola(agricola_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'AgricolaUpdateServlet.shxt?agricola_id='+agricola_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(agricola_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'AgricolaUploadServlet.shxt?agricola_id='+agricola_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(agricola_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeAgricolaStatusServlet.shxt",{agricola_id:agricola_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.agricola_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
...
<body>
			<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-comments"></i>农家乐管理
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="AgricolaServlet.shxt" method="post" id="myForm">
									<input type="text" name="agricola_name" id="agricola_name" value="${queryAgricola.agricola_name }" placeholder="请输入农家乐名称" maxlength="15">
									<select name="agricola_status" id="agricola_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryAgricola.agricola_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryAgricola.agricola_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addAgricola()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="agricola" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${agricola.agricola_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${agricola.agricola_name }
									</td>
									<td>
										 ${agricola.agricola_price }
									</td>
									<td>
										 ${agricola.agricola_address }
									</td>
									<td>
										 ${agricola.agricola_desc }
									</td>
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${agricola.agricola_id}',this)">
										 <c:if test="${agricola.agricola_status eq '1' }">存在</c:if>
										 <c:if test="${agricola.agricola_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${agricola.agricola_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateAgricola(${agricola.agricola_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
