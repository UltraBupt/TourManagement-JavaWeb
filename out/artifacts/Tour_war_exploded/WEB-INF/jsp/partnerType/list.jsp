...
<script type="text/javascript">
	...
	function addPartnerType(){
	   var addDialog = top.dialog({
		   title:'合作商类型添加',
		   url:'PartnerTypeAddServlet.shxt',
		   width:600,
		   height:215,
		   onclose:function(){
	...
	//编辑信息
	function updatePartnerType(partnerType_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'PartnerTypeUpdateServlet.shxt?partnerType_id='+partnerType_id,
			   width:600,
			   height:215,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
	function toChangeStatus(partnerType_id,obj){
		var txt = $.trim($(obj).html());
		if(window.confirm("您确定要改变状态么")){
			 $.post("ChangePartnerTypeStatusServlet.shxt",{partnerType_id:partnerType_id},function(data){
				 if(data.flag=="success"){
					 if(data.partnerType_status==1){
						  $(obj).html("禁用");
					 }else{
						 $(obj).html("可用");
					 }
...
<body>
			<div class="portlet box green">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-comments"></i>合作商类型管理
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								<form action="PartnerTypeListServlet.shxt" method="post" id="myForm">
									<input type="text" name="partnerType_name" id="partnerType_name" value="${queryPartnerType.partnerType_name }" placeholder="请输入合作商类型名称" maxlength="15">
									<select name="partnerType_status" id="partnerType_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryPartnerType.partnerType_status eq '1' }">
												selected
											</c:if>
										>可用</option>
										<option value="0"
											<c:if test="${queryPartnerType.partnerType_status eq '0' }">
												selected
											</c:if>
										>禁用</option>
									</select>
									<input type="submit" class="btn btn-circle btn-xs green" value="搜索" onclick="toSub(5,this)">
									...
									<shxt class="btn btn-circle btn-xs green" onclick="addPartnerType()">添加合作商类型</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="partnerType" varStatus="vs">
									<tr class="success">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 ${partnerType.partnerType_name }
									</td>
									<td>
										 ${partnerType.partnerType_desc }
									</td>
									<td>
									 	 <a href="javascript:void(0)" style="color:green" onclick="toChangeStatus('${partnerType.partnerType_id}',this)">
										 	<c:if test="${partnerType.partnerType_status eq '1' }">可用</c:if>
										 	<c:if test="${partnerType.partnerType_status eq '0' }">禁用</c:if>
										 </a>
									</td>
									<td>
										<shxt class="btn btn-circle btn-xs green" onclick="updatePartnerType(${partnerType.partnerType_id})">编辑合作商类型</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
								</table>
								<button class="btn btn-circle btn-xs green" onclick="toSub(1,this)">首页</button>
								<button class="btn btn-circle btn-xs green" onclick="toSub(2,this)">上一页</button>
								<button class="btn btn-circle btn-xs green" onclick="toSub(3,this)">下一页</button>
								<button class="btn btn-circle btn-xs green" onclick="toSub(4,this)">尾页</button><br/>
