...
function toSub(){
	...
	var ship_point=$("#ship_point").val();
	if(ship_from.trim().length==0){
		alert("请输入起点再进行提交");
		return false;
	}
	if(ship_to.trim().length==0){
		alert("请输入终点再进行提交");
		return false;
	}
	if(ship_price.trim().length==0){
		alert("请输入金额再进行提交");
		return false;
	}
	if(ship_time.trim().length==0){
		alert("请输入时间再进行提交");
		return false;
	}
	if(ship_point.trim().length==0){
		alert("请输入机场再进行提交");
	}
	$("#myForm").submit();
}	
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="ShipUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="起点" name="ship_from" id="ship_from" value="${ship.ship_from }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="终点" name="ship_to" id="ship_to" value="${ship.ship_to }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="ship_price" id="ship_price" value="${ship.ship_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="时间" name="ship_time" id="ship_time" value="${ship.ship_time }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="机场" name="ship_point" id="ship_point" value="${ship.ship_point }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="ship_id" name="ship_id" type="hidden" value="${ship.ship_id }"/>
								</div>
							</form>
						</div>
					</div>
