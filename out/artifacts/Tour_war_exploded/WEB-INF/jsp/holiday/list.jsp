...
<script type="text/javascript">
	...
	//编辑信息
	function updateHoliday(holiday_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'HolidayUpdateServlet.shxt?holiday_id='+holiday_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(holiday_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'HolidayUploadServlet.shxt?holiday_id='+holiday_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(holiday_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeHolidayStatusServlet.shxt",{holiday_id:holiday_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.holiday_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
...
<body>
			<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-comments"></i>假日酒店管理
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="HolidayServlet.shxt" method="post" id="myForm">
									<input type="text" name="holiday_name" id="holiday_name" value="${queryHoliday.holiday_name }" placeholder="请输入假日酒店名称" maxlength="15">
									<select name="holiday_status" id="holiday_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryHoliday.holiday_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryHoliday.holiday_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addHoliday()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="holiday" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px"> 
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${holiday.holiday_url }" style="width: 60px; height: 60px">
									</td>
									<td>
									...
									<td>
										 ${holiday.holiday_address }
									</td>
									<td>
										 ${holiday.holiday_desc }
									</td>
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${holiday.holiday_id}',this)">
										 <c:if test="${holiday.holiday_status eq '1' }">存在</c:if>
										 <c:if test="${holiday.holiday_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${holiday.holiday_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateHoliday(${holiday.holiday_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
