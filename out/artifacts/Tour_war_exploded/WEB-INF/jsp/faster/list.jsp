...
<script type="text/javascript">
	...
	//编辑信息
	function updateFaster(faster_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'FasterUpdateServlet.shxt?faster_id='+faster_id,
			   width:600,
			   height:350,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(faster_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'FasterUploadServlet.shxt?faster_id='+faster_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(faster_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeFasterStatusServlet.shxt",{faster_id:faster_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.faster_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
...
<body>
			<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-comments"></i>动车票管理
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="FasterServlet.shxt" method="post" id="myForm">
									<input type="text" name="faster_point" id="faster_point" value="${queryFaster.faster_point }" placeholder="请输入机场" maxlength="15">
									<select name="faster_status" id="faster_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryFaster.faster_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryFaster.faster_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addFaster()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="faster" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${faster.faster_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${faster.faster_from }
									</td>
									...
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${faster.faster_id}',this)">
										 <c:if test="${faster.faster_status eq '1' }">存在</c:if>
										 <c:if test="${faster.faster_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${faster.faster_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateFaster(${faster.faster_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
