...
<script type="text/javascript">
		...
		//变更状态
        function toChangeStatus(newMessage_id,obj){
        	var txt = $.trim($(obj).html());
        	if(window.confirm("您确定要改变状态么")){
        		 $.post("ChangeNewMessageStatusServlet.shxt",{newMessage_id:newMessage_id},function(data){
        			 if(data.flag=="success"){
        				 if(data.newMessage_status==1){
        					  $(obj).html("未处理");
        				 }else{
        					 $(obj).html("已处理");
        				 }
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="MessageListServlet.shxt" method="post" id="myForm">
									<select name="newMessage_status" id="newMessage_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryNewMessage.newMessage_status eq '1' }">
												selected
											</c:if>
										>已处理</option>
										<option value="0"
											<c:if test="${queryNewMessage.newMessage_status eq '0' }">
												selected
											</c:if>
										>未处理</option>
									</select>
								...
								<c:forEach items="${pageBean.data }" var="newMessage" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										${newMessage.newMessageMember_name }
									</td>
									<td>
										${newMessage.newMessageMember_account }
									</td>
									<td>
										${newMessage.newMessage_desc }
									</td>
									<td>
										${newMessage.newMessage_createDate }
									</td>
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${newMessage.newMessage_id}',this)">
										<c:if test="${newMessage.newMessage_status eq '1' }">已处理</c:if>
										<c:if test="${newMessage.newMessage_status eq '0' }">未处理</c:if>
									</a>
									</td>
								</tr>
								</c:forEach>
