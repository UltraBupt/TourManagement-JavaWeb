...
<script type="text/javascript">
	//添加招聘信息
		...
		//编辑信息
		function upload(recruit_id){
		   var updateDialog = top.dialog({
			   title:'招聘信息编辑',
			   url:'RecruitUpdateServlet.shxt?recruit_id='+recruit_id,
			   width:600,
			   height:415,
			   onclose:function(){
				  if(this.returnValue=="success"){
		...
		//变更状态
        function toChangeStatus(recruit_id,obj){
        	var txt = $.trim($(obj).html());
        	if(window.confirm("您确定要改变状态么")){
        		 $.post("ChangeRecruitStatusServlet.shxt",{recruit_id:recruit_id},function(data){
			...
			<div class="portlet box green">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="RecruitListServlet.shxt" method="post" id="myForm">
									<input type="text" name="recruit_company" id="recruit_company" value="${queryRecruit.recruit_company }" placeholder="请输入公司名称" maxlength="15">
									<select name="recruit_status" id="recruit_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryRecruit.recruit_status eq '1' }">
												selected
											</c:if>
										>可用</option>
										<option value="0"
											<c:if test="${queryRecruit.recruit_status eq '0' }">
												selected
											</c:if>
										>禁用</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs green" onclick="addRecruit()">添加招聘信息</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="recruit" varStatus="vs">
									<tr class="success">
									...
									<td>
										${recruit.recruit_desc }
									</td>
									<td>
										${recruit.recruit_benefit }
									</td>
									<td>
									...
									<td>
									<a href="javascript:void(0)" style="color:green" onclick="toChangeStatus('${recruit.recruit_id}',this)">
										<c:if test="${recruit.recruit_status eq '1' }">可用</c:if>
										<c:if test="${recruit.recruit_status eq '0' }">禁用</c:if>
									</a>
									</td>
									<td>
										<shxt class="btn btn-circle btn-xs green" onclick="upload(${recruit.recruit_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
