...
<script type="text/javascript">
	//上传头像
	function upload(inboundTicket_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'InboundTicketUploadServlet.shxt?inboundTicket_id='+inboundTicket_id,
			   width:600,
			   height:170,
			   onclose:function(){
	...
	//编辑信息
	function updateInboundTicket(inboundTicket_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'InboundTicketUpdateServlet.shxt?inboundTicket_id='+inboundTicket_id,
			   width:600,
			   height:240,
			   onclose:function(){
				  if(this.returnValue=="success"){
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								<form action="InboundTicketServlet.shxt" method="post" id="myForm">
								...
								<c:forEach items="${pageBean.data }" var="inboundTicket" varStatus="vs">
									<tr class="active">
									<td style="width: 91px; height: 91px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${inboundTicket.inboundTicket_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${inboundTicket.inboundTicket_desc }
									</td>
									<td>
										 ${inboundTicket.inboundTicket_price }
									</td>
									<td>
										 ${inboundTicket.inboundTicket_name }
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${inboundTicket.inboundTicket_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateInboundTicket(${inboundTicket.inboundTicket_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
