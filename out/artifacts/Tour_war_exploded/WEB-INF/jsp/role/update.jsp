...
<script type="text/javascript">
	function toSub(){
		...
		if(role_desc.trim().length==0){
			alert("角色描述不能为空，请重新输入");
			return false;
		}
		if(role_name.trim().length==0){
			alert("角色名称不能为空，请重新输入");
		}else{
			if(role_name.trim()==old_role_name){
				$("#myForm").submit();
			}else{
				$.post("CheckRoleNameServlet.shxt",{role_name:role_name},function(data){
					if(data.flg){
						$("#myForm").submit();
					}else{
						alert(data.message);
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 角色信息
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="RoleUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group" style="height:50px">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="角色名称" name="role_name" id="role_name" value="${role.role_name }">
										</div>
									</div>
									<div class="form-group" style="height:50px">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="角色描述" name="role_desc" id="role_desc" value="${role.role_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn red" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="role_id" name="role_id" type="hidden" value="${role.role_id }"/>
									<input id="old_role_name" name="old_role_name" type="hidden" value="${role.role_name }"/>
								</div>
							</form>
