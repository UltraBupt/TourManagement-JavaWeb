...
<script type="text/javascript">
	function toSub(){
		var snack_name=$("#snack_name").val();
		var snack_price=$("#snack_price").val();
		var snack_address=$("#snack_address").val();
		var snack_desc=$("#snack_desc").val();
		if(snack_name.trim().length==0){
			alert("请输入名称再进行提交");
			return false;
		}
		if(snack_price.trim().length==0){
			alert("请输入金额再进行提交");
			return false;
		}
		if(snack_address.trim().length==0){
			alert("请输入地址再进行提交");
			return false;
		}
		if(snack_desc.trim().length==0){
			alert("请输入描述再进行提交");
			return false;
		}
		$("#myForm").submit();
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="SnackUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="名称" name="snack_name" id="snack_name" value="${snack.snack_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="snack_price" id="snack_price" value="${snack.snack_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="地址" name="snack_address" id="snack_address" value="${snack.snack_address }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="详情" name="snack_desc" id="snack_desc" value="${snack.snack_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="snack_id" name="snack_id" type="hidden" value="${snack.snack_id }"/>
								</div>
							</form>
						</div>
					</div>
