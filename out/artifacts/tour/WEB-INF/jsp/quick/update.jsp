...
<script type="text/javascript">
	function toSub(){
		...
		var quick_address=$("#quick_address").val();
		var quick_desc=$("#quick_desc").val();
		if(quick_name.trim().length==0){
			alert("请输入名称再进行提交");
			return false;
		}
		if(quick_price.trim().length==0){
			alert("请输入金额再进行提交");
			return false;
		}
		if(quick_address.trim().length==0){
			alert("请输入地址再进行提交");
			return false;
		}
		if(quick_desc.trim().length==0){
			alert("请输入描述再进行提交");
			return false;
		}
		$("#myForm").submit();
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="QuickUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="名称" name="quick_name" id="quick_name" value="${quick.quick_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="quick_price" id="quick_price" value="${quick.quick_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="地址" name="quick_address" id="quick_address" value="${quick.quick_address }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="详情" name="quick_desc" id="quick_desc" value="${quick.quick_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="quick_id" name="quick_id" type="hidden" value="${quick.quick_id }"/>
								</div>
							</form>
						</div>
					</div>
