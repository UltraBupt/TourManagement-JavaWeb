...
<script type="text/javascript">
	...
	//编辑信息
	function updateQuick(quick_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'QuickUpdateServlet.shxt?quick_id='+quick_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(quick_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'QuickUploadServlet.shxt?quick_id='+quick_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(quick_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeQuickStatusServlet.shxt",{quick_id:quick_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.quick_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="QuickServlet.shxt" method="post" id="myForm">
									<input type="text" name="quick_name" id="quick_name" value="${queryQuick.quick_name }" placeholder="请输入快餐名称" maxlength="15">
									<select name="quick_status" id="quick_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryQuick.quick_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryQuick.quick_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addQuick()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="quick" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${quick.quick_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${quick.quick_name }
									</td>
									...
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${quick.quick_id}',this)">
										 <c:if test="${quick.quick_status eq '1' }">存在</c:if>
										 <c:if test="${quick.quick_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${quick.quick_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateQuick(${quick.quick_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
