...
function toSub(){
	...
	var bus_point=$("#bus_point").val();
	if(bus_from.trim().length==0){
		alert("请输入起点再进行提交");
		return false;
	}
	if(bus_to.trim().length==0){
		alert("请输入终点再进行提交");
		return false;
	}
	if(bus_price.trim().length==0){
		alert("请输入金额再进行提交");
		return false;
	}
	if(bus_time.trim().length==0){
		alert("请输入时间再进行提交");
		return false;
	}
	if(bus_point.trim().length==0){
		alert("请输入机场再进行提交");
	}
	$("#myForm").submit();
}	
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="BusUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="起点" name="bus_from" id="bus_from" value="${bus.bus_from }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="终点" name="bus_to" id="bus_to" value="${bus.bus_to }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="bus_price" id="bus_price" value="${bus.bus_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="时间" name="bus_time" id="bus_time" value="${bus.bus_time }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="机场" name="bus_point" id="bus_point" value="${bus.bus_point }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="bus_id" name="bus_id" type="hidden" value="${bus.bus_id }"/>
								</div>
							</form>
						</div>
					</div>
