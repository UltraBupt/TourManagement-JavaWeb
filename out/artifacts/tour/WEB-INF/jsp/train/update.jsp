...
function toSub(){
	var train_from=$("#train_from").val();
	var train_to=$("#train_to").val();
	var train_price=$("#train_price").val();
	var train_time=$("#train_time").val();
	var train_point=$("#train_point").val();
	if(train_from.trim().length==0){
		alert("请输入起点再进行提交");
		return false;
	}
	if(train_to.trim().length==0){
		alert("请输入终点再进行提交");
		return false;
	}
	if(train_price.trim().length==0){
		alert("请输入金额再进行提交");
		return false;
	}
	if(train_time.trim().length==0){
		alert("请输入时间再进行提交");
		return false;
	}
	if(train_point.trim().length==0){
		alert("请输入机场再进行提交");
	}
	$("#myForm").submit();
}	
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="TrainUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="起点" name="train_from" id="train_from" value="${train.train_from }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="终点" name="train_to" id="train_to" value="${train.train_to }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="train_price" id="train_price" value="${train.train_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="时间" name="train_time" id="train_time" value="${train.train_time }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="机场" name="train_point" id="train_point" value="${train.train_point }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="train_id" name="train_id" type="hidden" value="${train.train_id }"/>
								</div>
							</form>
						</div>
					</div>
