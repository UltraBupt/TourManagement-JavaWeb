...
<script type="text/javascript">
	function toSub(){
		...
		var villa_price=$("#villa_price").val();
		var villa_address=$("#villa_address").val();
		var villa_desc=$("#villa_desc").val();
		if(villa_name.trim().length==0){
			alert("请输入名称再进行提交");
			return false;
		}
		if(villa_price.trim().length==0){
			alert("请输入金额再进行提交");
			return false;
		}
		if(villa_address.trim().length==0){
			alert("请输入地址再进行提交");
			return false;
		}
		if(villa_desc.trim().length==0){
			alert("请输入描述再进行提交");
			return false;
		}
		$("#myForm").submit();
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="VillaUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="名称" name="villa_name" id="villa_name" value="${villa.villa_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="villa_price" id="villa_price" value="${villa.villa_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="地址" name="villa_address" id="villa_address" value="${villa.villa_address }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="详情" name="villa_desc" id="villa_desc" value="${villa.villa_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="villa_id" name="villa_id" type="hidden" value="${villa.villa_id }"/>
								</div>
							</form>
						</div>
					</div>
