...
<script type="text/javascript">
	function toSub(){
		var agricola_name=$("#agricola_name").val();
		var agricola_price=$("#agricola_price").val();
		var agricola_address=$("#agricola_address").val();
		var agricola_desc=$("#agricola_desc").val();
		if(agricola_name.trim().length==0){
			alert("请输入名称再进行提交");
			return false;
		}
		if(agricola_price.trim().length==0){
			alert("请输入金额再进行提交");
			return false;
		}
		if(agricola_address.trim().length==0){
			alert("请输入地址再进行提交");
			return false;
		}
		if(agricola_desc.trim().length==0){
			alert("请输入描述再进行提交");
			return false;
		}
		$("#myForm").submit();
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="AgricolaUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="名称" name="agricola_name" id="agricola_name" value="${agricola.agricola_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="agricola_price" id="agricola_price" value="${agricola.agricola_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="地址" name="agricola_address" id="agricola_address" value="${agricola.agricola_address }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="详情" name="agricola_desc" id="agricola_desc" value="${agricola.agricola_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="agricola_id" name="agricola_id" type="hidden" value="${agricola.agricola_id }"/>
								</div>
							</form>
						</div>
					</div>
