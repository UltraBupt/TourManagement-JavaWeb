...
<script type="text/javascript">
	function toSub(){
		...
		var apartment_price=$("#apartment_price").val();
		var apartment_address=$("#apartment_address").val();
		var apartment_desc=$("#apartment_desc").val();
		if(apartment_name.trim().length==0){
			alert("请输入名称再进行提交");
			return false;
		}
		if(apartment_price.trim().length==0){
			alert("请输入金额再进行提交");
			return false;
		}
		if(apartment_address.trim().length==0){
			alert("请输入地址再进行提交");
			return false;
		}
		if(apartment_desc.trim().length==0){
			alert("请输入描述再进行提交");
			return false;
		}
		$("#myForm").submit();
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="ApartmentUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="名称" name="apartment_name" id="apartment_name" value="${apartment.apartment_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="apartment_price" id="apartment_price" value="${apartment.apartment_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="地址" name="apartment_address" id="apartment_address" value="${apartment.apartment_address }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="详情" name="apartment_desc" id="apartment_desc" value="${apartment.apartment_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="apartment_id" name="apartment_id" type="hidden" value="${apartment.apartment_id }"/>
								</div>
							</form>
						</div>
					</div>
