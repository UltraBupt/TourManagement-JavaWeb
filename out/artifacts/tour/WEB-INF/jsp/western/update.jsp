...
<script type="text/javascript">
	function toSub(){
		var western_name=$("#western_name").val();
		var western_price=$("#western_price").val();
		var western_address=$("#western_address").val();
		var western_desc=$("#western_desc").val();
		if(western_name.trim().length==0){
			alert("请输入名称再进行提交");
			return false;
		}
		if(western_price.trim().length==0){
			alert("请输入金额再进行提交");
			return false;
		}
		if(western_address.trim().length==0){
			alert("请输入地址再进行提交");
			return false;
		}
		if(western_desc.trim().length==0){
			alert("请输入描述再进行提交");
			return false;
		}
		$("#myForm").submit();
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 编辑
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="WesternUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="名称" name="western_name" id="western_name" value="${western.western_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="number" class="form-control input-sm" placeholder="金额" name="western_price" id="western_price" value="${western.western_price }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="地址" name="western_address" id="western_address" value="${western.western_address }" >
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="详情" name="western_desc" id="western_desc" value="${western.western_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn blue" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="western_id" name="western_id" type="hidden" value="${western.western_id }"/>
								</div>
							</form>
						</div>
					</div>
