...
<script type="text/javascript">
	...
	//编辑信息
	function updateWestern(western_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'WesternUpdateServlet.shxt?western_id='+western_id,
			   width:600,
			   height:300,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(western_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'WesternUploadServlet.shxt?western_id='+western_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(western_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeWesternStatusServlet.shxt",{western_id:western_id},function(data){
    			 if(data.flag=="success"){
    				 if(data.western_status==1){
    					  $(obj).html("不存在");
    				 }else{
    					 $(obj).html("存在");
    				 }
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="WesternServlet.shxt" method="post" id="myForm">
									<input type="text" name="western_name" id="western_name" value="${queryWestern.western_name }" placeholder="请输入西式名称" maxlength="15">
									<select name="western_status" id="western_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryWestern.western_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryWestern.western_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addWestern()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="western" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${western.western_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${western.western_name }
									</td>
									<td>
										 ${western.western_price }
									</td>
									<td>
										 ${western.western_address }
									</td>
									<td>
										 ${western.western_desc }
									</td>
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${western.western_id}',this)">
										 <c:if test="${western.western_status eq '1' }">存在</c:if>
										 <c:if test="${western.western_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${western.western_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateWestern(${western.western_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
