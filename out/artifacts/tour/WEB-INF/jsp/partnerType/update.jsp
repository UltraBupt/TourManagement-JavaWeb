...
<script type="text/javascript">
	function toSub(){
		var partnerType_name=$("#partnerType_name").val();
		var old_partnerType_name=$("#old_partnerType_name").val();
		var partnerType_desc=$("#partnerType_desc").val();
		if(partnerType_desc.trim().length==0){
			alert("供应商类型描述不能为空，请重新输入");
			return false;
		}
		if(partnerType_name.trim().length==0){
			alert("供应商名称不能为空，请重新输入");
		}else{
			if(partnerType_name.trim()==old_partnerType_name){
				$("#myForm").submit();
			}else{
				$.post("CheckPartnerTypeNameServlet.shxt",{partnerType_name:partnerType_name},function(data){
					if(data.flg){
						$("#myForm").submit();
					}else{
						alert(data.message);
						$("#partnerType_name").focus();
					}
				},"json");
			}
		}
				...
				<div class="caption">
					<i class="fa fa-gift"></i> 供应商类型信息
						...
						<div class="portlet-body form">
							...
							<form class="form-horizontal" action="PartnerTypeUpdateServlet.shxt" method="post" id="myForm">
								<div class="form-body">
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="供应商类型名称" name="partnerType_name" id="partnerType_name" value="${partnerType.partnerType_name }">
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-9">
											<input type="text" class="form-control input-sm" placeholder="供应商类型描述" name="partnerType_desc" id="partnerType_desc" value="${partnerType.partnerType_desc }">
										</div>
									</div>
								</div>
								<div class="form-actions right1">
									<button type="button" class="btn green" value="修改信息" onclick="toSub()">修改信息</button>
									<input id="partnerType_id" name="partnerType_id" type="hidden" value="${partnerType.partnerType_id }"/>
									<input id="old_partnerType_name" name="old_partnerType_name" type="hidden" value="${partnerType.partnerType_name }"/>
								</div>
							</form>
