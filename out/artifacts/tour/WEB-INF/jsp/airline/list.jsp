...
<script type="text/javascript">
	...
	function updateAirline(airline_id){
		   var updateDialog = top.dialog({
			   title:'编辑信息',
			   url:'AirlineUpdateServlet.shxt?airline_id='+airline_id,
			   width:600,
			   height:350,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//上传图片
	function upload(airline_id){
		   var uploadDialog = top.dialog({
			   title:'上传图片',
			   url:'AirlineUploadServlet.shxt?airline_id='+airline_id,
			   width:600,
			   height:170,
			   onclose:function(){
				  if(this.returnValue=="success"){
	...
	//变更状态
    function toChangeStatus(airline_id,obj){
    	var txt = $.trim($(obj).html());
    	if(window.confirm("您确定要改变状态么")){
    		 $.post("ChangeAirlineStatusServlet.shxt",{airline_id:airline_id},function(data){
			...
			<div class="portlet box blue">
						...
						<div class="portlet-body">
							<div class="table-scrollable">
								...
								<form action="AirlineServlet.shxt" method="post" id="myForm">
									<input type="text" name="airline_point" id="airline_point" value="${queryAirline.airline_point }" placeholder="请输入机场" maxlength="15">
									<select name="airline_status" id="airline_status" class="btn btn-sm dropdown-toggle selectpicker">
										<option value="">全部</option>
										<option value="1"
											<c:if test="${queryAirline.airline_status eq '1' }">
												selected
											</c:if>
										>存在</option>
										<option value="0"
											<c:if test="${queryAirline.airline_status eq '0' }">
												selected
											</c:if>
										>不存在</option>
									</select>
									...
									<shxt class="btn btn-circle btn-xs blue" onclick="addAirline()">添加</shxt>
								</form>
								...
								<c:forEach items="${pageBean.data }" var="airline" varStatus="vs">
									<tr class="active">
									<td style="width: 84px; height: 84px">
										 ${vs.count+(pageBean.pageNow-1)*pageBean.pageSize }
									</td>
									<td>
										 <img alt="" src="${airline.airline_url }" style="width: 60px; height: 60px">
									</td>
									<td>
										 ${airline.airline_from }
									</td>
									<td>
										 ${airline.airline_to }
									</td>
									<td>
										 ${airline.airline_price }
									</td>
									<td>
										 ${airline.airline_time }
									</td>
									<td>
										 ${airline.airline_point }
									</td>
									<td>
									<a href="javascript:void(0)" style="color:blue" onclick="toChangeStatus('${airline.airline_id}',this)">
										 <c:if test="${airline.airline_status eq '1' }">存在</c:if>
										 <c:if test="${airline.airline_status eq '0' }">不存在</c:if>
									</a>
									</td>
									<td>
										 <shxt class="btn btn-circle btn-xs blue" onclick="upload(${airline.airline_id})">上传图片</shxt><br/>
										 <shxt class="btn btn-circle btn-xs blue" onclick="updateAirline(${airline.airline_id})">编辑信息</shxt><br/>
									</td>
								</tr>
								</c:forEach>
								</tbody>
